...
このフォルダ一式を、わかりやすい場所に置きます。おすすめは次の場所です。

```
C:\Users\<あなたのユーザー名>\TranscribeJA
```

> **注意:** OneDrive で同期されるフォルダ（デスクトップ、ドキュメントなど）は
> 避けたほうが安全です。同期のたびに大きなファイルが転送されてしまいます。

### 手順 2: インストールを実行する

1. 置いたフォルダを **エクスプローラー** で開きます
2. アドレス欄（フォルダの場所が書いてある欄）をクリックして `powershell` と入力し、
   **Enter** を押します。そのフォルダで PowerShell が開きます
3. 次の 1 行を貼り付けて **Enter** を押します

   ```powershell
   Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force; .\install.ps1
   ```

   > 前半の `Set-ExecutionPolicy ...` は、この PowerShell の窓の中でだけ
   > スクリプトの実行を許可する指定です。パソコン全体の設定は変わりません。

4. 画面の案内に従って待ちます。**5〜20 分ほど**かかります
